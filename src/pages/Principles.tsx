      ...
      <section className="py-20 sm:py-24">
        <div className="section-shell grid gap-5 md:grid-cols-2">
          {principles.map((principle, index) => (
            <article key={principle.title} className="panel p-7">
              <p className="text-sm font-black text-coral">0{index + 1}</p>
              <h2 className="mt-4 text-3xl font-black text-white">{principle.title}</h2>
              <p className="mt-4 leading-8 text-mist">{principle.body}</p>
            </article>
          ))}
        </div>
      </section>
